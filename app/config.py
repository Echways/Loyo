...
class Settings(BaseSettings):
    ...
    REDIS_DSN: str = Field(
        "redis://redis:6379/0", description="Redis DSN used by the app"
    )
    ...
    ADMINS: str = Field(
        "", description="Comma-separated list of admin Telegram IDs, e.g. '123,456'"
    )
    ...
    def admin_ids(self) -> List[int]:
        if not self.ADMINS:
            return []
        parts = [
            p.strip() for p in self.ADMINS.replace(" ", ",").split(",") if p.strip()
        ]
        ids: List[int] = []
        for p in parts:
            try:
