...
async def build_catalog_markup(
    node: Dict, include_back: bool = False
    ...
    for it in node.get("items", []):
        if it.get("type") == "category":
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        text=f"📁 {it['title']}", callback_data=cb.pack()
                    )
                ]
            )
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        text=await product_button_text(it), callback_data=cb.pack()
                    )
                ]
            )
...
def build_admin_confirm_kb(purchase_id: str) -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Подтвердить оплату", callback_data=cb.pack()
                )
            ]
        ]
    )
