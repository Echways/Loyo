...
def product_button_text(prod: Dict) -> str:
    price_text = f" — {prod.get('price')}₽" if prod.get('price') else ""
    return f"🛍 {prod.get('title')}{price_text}"

def build_catalog_markup(node: Dict, include_back: bool = False) -> InlineKeyboardMarkup:
    keyboard = []
    for it in node.get("items", []):
        if it.get("type") == "category":
            ...
            keyboard.append([InlineKeyboardButton(text=product_button_text(it), callback_data=cb.pack())])
