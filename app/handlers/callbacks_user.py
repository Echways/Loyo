...
def get_callbacks_user_router(async_session_maker, ranks) -> Router:
    ...
    async def profile_cb(
        query: types.CallbackQuery, callback_data: UserProfileCallback
    ):
        if callback_data.action == "rank_progress":
            ...
            async with get_session(async_session_maker) as session:
                ...
                rank_range = await ranks.get_rank_points_range_by_points(
                    user.rank_points
                )
                next_rank = (
                    await ranks.get_rank_by_points(rank_range[1] + 1)
                    if rank_range[1] != 0
                    else None
                )
                ...
                if next_rank:
                    await query.answer(
                        f"{rank_range[0]} --- {user.rank_points} --- {rank_range[1]} \nСледующий ранг: {next_rank.name}",
                        show_alert=True,
                    )
                else:
